...
const LEAGUES = String(process.env.SPORTS_WORKFLOW_CANARY_LEAGUES || "nba,nfl,mlb,nhl")
  .split(",")
...
function buildPayload(league, seed = DEFAULT_SEED) {
  const teams = LEAGUE_TEAMS[league];
  if (!teams) {
    throw new Error(`Unsupported league in canary payload builder: ${league}`);
  }
...
async function postJson(baseUrl, league, body) {
  const response = await fetch(`${baseUrl}/api/workflows/sports/${league}/championship-forecast`, {
    method: "POST",
...
async function run() {
  ...
  console.log("=== Sports Workflow Canary ===");
  console.log(`Seed: ${DEFAULT_SEED}`);
  console.log(`Simulations: ${DEFAULT_SIMS}`);
  console.log(`Leagues: ${LEAGUES.join(", ")}\n`);

  await withServer(app, async (baseUrl) => {
    for (const league of LEAGUES) {
      const baseline = await postJson(baseUrl, league, buildPayload(league, DEFAULT_SEED));
      const repeat = await postJson(baseUrl, league, buildPayload(league, DEFAULT_SEED));
      const differentSeed = await postJson(baseUrl, league, buildPayload(league, DEFAULT_SEED + 1));
      const expectedPath = `outputs/${league}-championship-forecast-2026-04-03.xlsx`;
      ...
      const checks = [
        ...
        },
        {
          ...
          ok: JSON.stringify(sanitizeForDeterministicCompare(baseline.payload)) === JSON.stringify(sanitizeForDeterministicCompare(repeat.payload)),
          detail: "baseline payload did not match repeat payload",
        },
        {
          name: "different seed changes output",
          ok: JSON.stringify(sanitizeForDeterministicCompare(baseline.payload)) !== JSON.stringify(sanitizeForDeterministicCompare(differentSeed.payload)),
          detail: "baseline payload matched different-seed payload",
        },
        {
          name: "prediction winner is present",
          ok: typeof baseline.payload?.prediction?.predicted_winner === "string" && baseline.payload.prediction.predicted_winner.length > 0,
          detail: `winner was ${JSON.stringify(baseline.payload?.prediction?.predicted_winner ?? null)}`,
        },
        {
          name: "ranking is non-empty",
          ok: Array.isArray(baseline.payload?.ranking) && baseline.payload.ranking.length > 0,
          detail: `ranking length was ${baseline.payload?.ranking?.length ?? 0}`,
        },
        {
          name: "xlsx artifact is bundled when requested",
          ok: baseline.payload?.artifacts?.xlsx?.documentType === "xlsx",
          detail: `xlsx artifact was ${JSON.stringify(baseline.payload?.artifacts?.xlsx ?? null)}`,
        },
        {
          name: "xlsx recommended path is present",
          ok: baseline.payload?.artifacts?.xlsx?.recommended_local_path === expectedPath,
          detail: `recommended path was ${JSON.stringify(baseline.payload?.artifacts?.xlsx?.recommended_local_path ?? null)}`,
        }
      ];
