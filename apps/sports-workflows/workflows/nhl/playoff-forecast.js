const snapshot = require("../../fixtures/nhl-standings-2026-04-03.json");
const { runBatchProbability } = require("../../../generic-parameter-simulator/sim/engine");
const { NHL_ASSUMPTIONS } = require("./defaults");
const { selectTeams, teamToScenario } = require("./normalize");
const { buildNhlPlayoffForecastReport } = require("../../lib/report");
...
async function runNhlPlayoffForecast(params) {
  ...
  const workflowResult = {
    ...
    ranking: rankedTeams,
    assumptions: NHL_ASSUMPTIONS[params.field] || NHL_ASSUMPTIONS.top_6_only,
    diagnostics: {
  ...
  const report = buildNhlPlayoffForecastReport(workflowResult);
  ...
  if (params.includeArtifacts.length > 0) {
    workflowPayload.artifacts = await attachRequestedArtifacts(
      workflowPayload,
      params.reportTitle || "NHL Championship Forecast",
