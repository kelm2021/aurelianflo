const snapshot = require("../../fixtures/mlb-standings-2026-04-03.json");
const { runBatchProbability } = require("../../../generic-parameter-simulator/sim/engine");
const { MLB_ASSUMPTIONS } = require("./defaults");
const { selectTeams, teamToScenario } = require("./normalize");
const { buildMlbPlayoffForecastReport } = require("../../lib/report");
...
async function runMlbPlayoffForecast(params) {
  ...
  const workflowResult = {
    ...
    ranking: rankedTeams,
    assumptions: MLB_ASSUMPTIONS[params.field] || MLB_ASSUMPTIONS.top_6_only,
    diagnostics: {
  ...
  const report = buildMlbPlayoffForecastReport(workflowResult);
  ...
  if (params.includeArtifacts.length > 0) {
    workflowPayload.artifacts = await attachRequestedArtifacts(
      workflowPayload,
      params.reportTitle || "MLB Championship Forecast",
